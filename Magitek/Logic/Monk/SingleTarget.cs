...
namespace Magitek.Logic.Monk
{
    public class SingleTarget
    {
        ...
        {
            if (Core.Me.ClassLevel < Spells.Bootshine.LevelAcquired)
                return false;

            if (Core.Me.ClassLevel >= Spells.FormShift.LevelAcquired && !Core.Me.HasAura(Auras.OpoOpoForm))
                return false;

            if (Core.Me.ClassLevel >= Spells.DragonKick.LevelAcquired && Core.Me.ClassLevel < Spells.FormShift.LevelAcquired
                && !Core.Me.HasAura(Auras.LeadenFist))
                return false;
        ...
        public static async Task<bool> TrueStrike()
        {
            ...
            if (!Core.Me.HasAura(Auras.DisciplinedFist) && Core.Me.ClassLevel >= Spells.TwinSnakes.LevelAcquired)
                return false;
        ...
        public static async Task<bool> SnapPunch()
        {
            ...
            if (!Core.Me.CurrentTarget.HasAura(Auras.Demolish) && Core.Me.ClassLevel >= Spells.Demolish.LevelAcquired)
                return false;
        ...
        public static async Task<bool> PerfectBalanceRoT()
        {
            ...
            if (ActionResourceManager.Monk.ActiveNadi.HasFlag(Nadi.Both)
                || (!ActionResourceManager.Monk.ActiveNadi.HasFlag(Nadi.Both)
                    && ActionResourceManager.Monk.ActiveNadi.HasFlag(Nadi.Solar)))
            {
